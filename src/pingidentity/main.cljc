(ns pingidentity.main
  "Kotodama WASM entrypoint for the Pingidentity clean-room actor (L4) — Clojure port.
  ...
  facts (`pingidentity.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pingidentity")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"pingidentity.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pingidentity-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pingidentity-compat/ready)
